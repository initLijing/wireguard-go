...
func (d *lazyDLL) Load() error {
	...
	size, err := resource.Sizeof(ourModule, resInfo)
	if err != nil {
		return fmt.Errorf("Unable to size resource: %v", err)
	}
	resData, err := resource.Load(ourModule, resInfo)
	if err != nil {
		return fmt.Errorf("Unable to load resource: %v", err)
	}
	addr, err := resource.Lock(resData)
	if err != nil {
		return fmt.Errorf("Unable to lock resource: %v", err)
	}
	module, err := memmod.LoadLibrary(addr, uintptr(size))
	if err != nil {
